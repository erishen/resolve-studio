...
- **Pre-commit gate**: `make hook-init` enables a pre-commit hook (`.githooks/pre-commit`) that guards two things on commit — it blocks staged secrets via `gitleaks protect --staged` (skipped when gitleaks is absent) and checks the staged files against the Prettier rules with `prettier --check --ignore-unknown` (skipped when `node_modules/prettier` is absent). Set `SKIP_PRETTIER_CHECK=1` or pass `--no-verify` to bypass. `make secret-scan` instead scans the whole repo on demand. The hook only lives in `.githooks/`, so each fresh clone needs `make hook-init` once.
