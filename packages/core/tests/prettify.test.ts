...
test('linkifyArtifactHtml turns an in-sandbox .html path into a /api/raw link', () => {
  const text =
    '📊 总览已生成: <PROJECT_ROOT>/frameworks/llamaindex-pse/tasks/hot-news/hot-news-overview.html'
  ...
  assert.ok(!out.includes('<HOME>/Workspace'), 'long fs path hidden behind the link')
})
...
test('tidyToolEcho collapses → /abs/path directory echoes to the trailing name', () => {
  const text =
    'hot-news-fetch 完成 → <PROJECT_ROOT>/frameworks/llamaindex-pse/tasks/hot-news/news（全部源）'
...
test('prettifyAnswer cleans the real hot-news success bubble', () => {
  const raw = [
    'hot-news-fetch 完成 → <PROJECT_ROOT>/frameworks/llamaindex-pse/tasks/hot-news/news（全部源）',
    '▶ 抓取热点新闻 → <PROJECT_ROOT>/frameworks/llamaindex-pse/tasks/hot-news/news（直连）',
    '✅ 完成：新增/更新 98 条，清理旧文件 88 个 → <PROJECT_ROOT>/frameworks/llamaindex-pse/tasks/hot-news/news',
    '📊 总览已生成: <PROJECT_ROOT>/frameworks/llamaindex-pse/tasks/hot-news/hot-news-overview.html',
  ].join('\n')
  ...
  assert.ok(!out.includes('<PROJECT_ROOT>/frameworks'), 'no long path leaks')
})
