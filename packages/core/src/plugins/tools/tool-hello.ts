/**
 * `hello` tool — exposes the @resolve-studio/plugin-hello service to the agent.
 *
...
// @resolve-studio/plugin-hello (a separate package); we only rely on its shape.
  ...
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        greeting: {
          type: 'string',
          description: 'Optional custom greeting; defaults to the service greeting.',
        },
      },
    },
    async execute(args) {
