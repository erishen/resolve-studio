...
const registerAnalyzeCodeDir = (ctx: Context) => {
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        dir: {
          type: 'string',
          description:
            'Directory path (absolute, or relative to the harness working directory) to analyze.',
        },
        maxFiles: {
          type: 'number',
    ...
    async execute(args) {
      ...
      try {
        const act = await ctx.tools.call(
        ...
        if (isError(raw)) {
          ...
          try {
            ...
            report = {
              ...
              usedTools: ['analyze-dir'],
              dir: abs,
              rawResult: raw.slice(0, OUTPUT_BUDGET),
            }
          }
        }
      }
...
export const toolAnalyzeCodeDir = definePlugin(registerAnalyzeCodeDir, 'tool-analyze-code-dir', [
  'tools',
  'fsRoots',
])
