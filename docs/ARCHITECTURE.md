...
> 一份关于 `resolve-studio` 运行时设计与实现的说明，对应源码 `packages/core/src/`。
...
装配链路由四个文件协作完成，全部由 `packages/core/src/index.ts` 引导：
...
`chatStream` 为可选流式方法，默认 fallback 到 `chat` 一次吐全量（见 §4 事件总线的 `llm/stream`）。
